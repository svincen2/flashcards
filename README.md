...
* [`src/clj/flashcards/`](src/clj/flashcards/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/clj/flashcards/`](src/clj/flashcards/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/flashcards/`](src/cljs/flashcards/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/flashcards/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `flashcards` project root directory
...
in the [`src/clj/flashcards/`](src/clj/flashcards/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/flashcards/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns flashcards.example
  (:require [flashcards.config :as config])
...
```sh
port=2000 java -jar target/flashcards.jar
```
